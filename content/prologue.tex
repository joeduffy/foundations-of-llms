...
These capabilities arose from next-token prediction at scale rather than from explicit engineering, suggesting that the statistical structure of natural language encodes more about general cognition than had been previously assumed.

\section*{Alignment, Reasoning, and Agency}

A pretrained model predicts text; it does not yet follow instructions, verify its own reasoning, or act in an environment. Closing each of these gaps meant extending the same optimization principle beyond next-token prediction, training the model against a new signal rather than a new algorithm. Christiano and colleagues, then Ouyang and colleagues, showed that comparisons between human-ranked outputs could supply this signal directly, producing models that followed instructions rather than merely continuing them. Rafailov and colleagues simplified the resulting machinery, and Shao and colleagues then redirected it toward a different kind of signal: reward that could be checked mechanically---a correct proof, a passing test---rather than judged. Under this regime, extended reasoning stopped being a behavior elicited by careful prompting and became one the model learned to produce and verify on its own.

The same shift, from predicting an answer to acting on it, extended once more. Early agents were built by prompting and scaffolding a fixed model to call tools, retry after failure, and reflect on its own mistakes, engineering the interface between model and environment as carefully as the model itself. Later agents were trained directly against the outcomes of their actions in an environment, using the same reinforcement-learning machinery that produced reasoning models but applied now to whether a search returned a useful result or a multi-step task was completed, rather than to a single graded response. The lineage traced in this volume runs from a single artificial neuron to a system that plans, calls tools, and revises its own output: one continuous extension of the same learning principle to progressively less constrained settings.

\section*{Organization}

This volume organizes its papers into eight chronological parts and two appendices. Each part opens with a brief introduction situating the era in its intellectual context, followed by individual paper summaries and the papers themselves in their original form. Two appendices address active research frontiers: alternatives to the dense transformer, interpretability, and sparse-model scaling; and the foundations of autonomous agents. Selection favors the foundational papers that introduced each technique; integrated model reports and system cards, however consequential, are cited in the introductions and summaries rather than reproduced.
...
The papers collected here document an open line of investigation. Attention, self-supervised pretraining, preference-based and verifiable-reward optimization, and mixture-of-experts routing all originate in this collection and continue to define the research frontier. Understanding their origins equips the reader to evaluate what comes next.
