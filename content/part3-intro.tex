\PartIntroChapter{Introduction to Part III}
...
The deep learning revolution began with AlexNet's performance on ImageNet in 2012, which demonstrated that convolutional networks trained on GPUs could surpass all prior approaches to visual recognition.\index{convolutional neural network} This result validated decades of theoretical work on deep architectures and triggered rapid progress in network design. Batch normalization\index{batch normalization} stabilized deep network training by normalizing layer inputs, an effect its authors attributed to the reduction of internal covariate shift---a mechanism later work disputed without displacing the technique. ResNet's skip connections solved the degradation problem in very deep networks\index{residual connections} by allowing gradients to flow through shortcut paths---a design principle that would later appear in every transformer. The Adam optimizer provided adaptive per-parameter learning rates\index{Adam optimizer} that became the default for training large models. Dropout offered a practical regularization method grounded in ensemble averaging.\index{dropout} Hinton, Vinyals, and Dean showed that the function learned by a large model or ensemble could be transferred to a smaller model by training it against the larger model's softened output distribution---knowledge distillation, a technique that a decade later would underpin\index{knowledge distillation} the production of small frontier models from large ones.

In parallel, the sequence-to-sequence paradigm established the encoder-decoder framework for mapping variable-length inputs to variable-length outputs through a fixed-dimensional bottleneck. Cho introduced the GRU, a simplified recurrent unit, and formalized the encoder-decoder architecture. Sutskever demonstrated that this framework could learn complex transductions end-to-end. Bahdanau then identified the critical weakness of fixed-length encodings---they forced the encoder to compress all source information into a single vector---and proposed the attention mechanism, which allowed the decoder to selectively attend\index{attention mechanism} to different portions of the input at each generation step. Pennington and colleagues meanwhile fused the two dominant embedding traditions, fitting global co-occurrence statistics with a local-window objective to produce the GloVe word vectors. Byte-pair encoding provided the subword tokenization scheme that balanced vocabulary coverage with computational tractability, becoming the standard input representation for language models.
...
\item \textbf{Hinton, Vinyals \& Dean (2015)} --- Knowledge distillation: transferring a large model's learned function to a smaller model through softened output targets.
