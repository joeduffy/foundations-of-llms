...
\renewcommand{\maketitle}{%
  \begin{titlepage}%
    ...
    \let \footnote \thanks
    % A single thin gold rule inset from the trim -- the frame of a
    % gold-tooled binding, drawn once, restrained rather than doubled or
    % ornamented at the corners.
    \begin{tikzpicture}[remember picture, overlay]
      \draw[titlegold, line width=0.9pt]
        ([xshift=1.15cm,yshift=1.15cm]current page.south west) rectangle
        ([xshift=-1.15cm,yshift=-1.15cm]current page.north east);
    \end{tikzpicture}%
    \null\vfil
    \vskip 40\p@
    \begin{center}%
      % Subtle neural network diagram, reversed for the dark field
      \begin{tikzpicture}[scale=0.6, transform shape]
        \tikzset{
          neuron/.style={circle, draw=white!55, fill=white!12, minimum size=8pt, inner sep=1pt},
          connection/.style={draw=white!35, line width=0.5pt}
        }
        % Input layer
      ...
      \begin{tikzpicture}[baseline={(0,-0.15)}]
        \draw[titlegold, line width=0.7pt] (0,0) circle (5.5mm);
        \draw[titlegold, line width=0.35pt] (0,0) circle (4.6mm);
        \foreach \angle in {0,60,...,300} {
          \draw[titlegold, line width=0.3pt] (0,0) -- (\angle:3.6mm);
          \fill[titlegold] (\angle:3.6mm) circle (0.35mm);
        }
        \draw[titlegold, line width=0.25pt]
          (0:3.6mm) -- (60:3.6mm) -- (120:3.6mm) -- (180:3.6mm)
          -- (240:3.6mm) -- (300:3.6mm) -- cycle;
        \fill[titlegold] (0,0) circle (0.4mm);
      \end{tikzpicture}\\[9pt]
      ...
      {\normalsize\color{white!75}by Joe Duffy}
    \end{center}\par
    \@thanks
    \vfil\null
  \end{titlepage}%
