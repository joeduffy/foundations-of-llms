...
% A part's introduction and its papers are siblings -- both belong
% directly to the part, and neither is nested inside the other -- so
% both sit at the same indent, one step in from the part itself.
% Distinguishing them by font size (below) rather than by depth keeps
% that sibling relationship visible instead of implying the papers are
% children of the introduction. Prologue, Revision History, Epilogue,
% and the Index stay flush with the part, since they belong to the book
% as a whole rather than to any single part.
